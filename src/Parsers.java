...
public class Parser {
	...
	public static ArrayList<String> getMethodNames(List<String> fileLines, ArrayList<Integer> scopeBracketLines) {
		...
		while (lineNumber < fileLines.size()) {
			...
			lineNumber = nextNonNestedLineNumber(innerScopeLines, lineNumber, fileLines.size());
		}
		return methodNames;
	...
	public static HashSet<Variable> getScopeVariables(List<String> fileLines,
		...
		while (lineNumber < scopeEndLine) {
			...
			lineNumber = nextNonNestedLineNumber(innerScopeLines, lineNumber, scopeEndLine);
		}
	 ...
	 */
	private static int nextNonNestedLineNumber(ArrayList<Integer> innerScopeLines, int lineNumber, int scopeSneLine) {
		// the interval of indexes in the bracket array between two scopes
		final int ARRAY_INTERVAL = 2;
		
		for (int i = 0; i < scopeSneLine; i = i + ARRAY_INTERVAL) {
